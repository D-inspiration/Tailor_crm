...
class FlaskSessionMiddleware:
    ...
    def __call__(self, request):
        ...
        # If validation denied, redirect to login
        ...
        # Check if blocked — force logout
        if response.get("risk_status") == "blocked":
            from django.contrib.auth import logout
            from django.contrib import messages
            logout(request)
            request.session.flush()
            messages.error(request, "Account locked due to suspicious activity. Contact support.")
            from django.shortcuts import redirect
            return redirect('login')
        
        # Store risk data for badge
        ...
        print("MIDDLEWARE DATA:", dict(request.session))
