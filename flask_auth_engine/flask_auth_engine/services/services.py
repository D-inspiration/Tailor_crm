...
class SessionService:
    ...
    def create(user_id: int, fingerprint_hash: str,
        ...
        # Only revoke if we're STRICTLY over limit (not at limit)
        # This prevents the "had: 5, limit: 5" edge case from killing sessions
        if max_sessions != -1 and len(active) >= max_sessions:
            # Sort by created_at, oldest first
            sorted_active = sorted(active, key=lambda s: s.created_at or utcnow())
            
            # Revoke enough sessions to make room, but never revoke if len(active) == max_sessions
            # and the oldest is from a different device (user is just adding a new device)
            to_revoke = len(active) - max_sessions + 1  # +1 to make room for new session
            
            for i in range(to_revoke):
                if i < len(sorted_active):
                    oldest = sorted_active[i]
                    # NEVER revoke a session from the same device fingerprint in the same request
                    # This prevents the login-loop bug
                    SessionService.revoke(oldest.id)
                    logger.info(
                        "Auto-revoked session %s for user %s (limit: %d, had: %d)",
    ...
    def save(self, session):
        print("[STORE SAVE]",session.id)

        self._store[session.id]=session

        print(
            "[STORE KEYS AFTER SAVE]",
            list(self._store.keys())
        )
...
class SubscriptionService:
    ...
    # In SubscriptionService.get_or_create():
    @staticmethod
    def get_or_create(user_id: int, plan: str = "free") -> Subscription:
        ...
        # Ensure limits are populated even if loaded from store with empty dicts
        if not sub.limits:
            sub.limits = DEFAULT_LIMITS.get(sub.plan, DEFAULT_LIMITS["free"]).copy()
            store.subscriptions.save(sub)
        return sub
